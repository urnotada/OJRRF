...
def evaluate_all(model, dataloader, device):
    ...
    n_cf_batch = dataloader.n_cf_test // dataloader.cf_batch_size + 1
    pred_label = []
    pred_true=[]
    for iter in range(1, n_cf_batch + 1):
        time2 = time()
        cf_batch_user, cf_batch_pos_item, cf_batch_neg_item = dataloader.generate_cf_batch(dataloader.test_user_dict,
        ...
        with torch.no_grad():
            batch_scores_pos = model(cf_batch_user,cf_batch_pos_item, mode='predict')  # (n_batch_users, n_items)
            batch_scores_neg = model(cf_batch_user, cf_batch_neg_item, mode='predict')  # (n_batch_users, n_items)
        batch_scores_pos = batch_scores_pos.cpu().detach().view(-1).numpy().tolist()

        batch_scores_neg=batch_scores_neg.cpu().detach().view(-1).numpy().tolist()


        pred_label.extend(batch_scores_pos)
        pred_label.extend(batch_scores_neg)
        pred_true.extend([ 1 for i in range(len(batch_scores_pos))]) # 1
        pred_true.extend([ 0 for i in range(len(batch_scores_pos))]) # 0
    auc = roc_auc_score(pred_true, pred_label)  # auc
    ...
    # p,recall,f1
    precision=precision_score(pred_true,pred_label)
    recall=recall_score(pred_true,pred_label)
    f1=f1_score(pred_true,pred_label)
    acc=accuracy_score(pred_true,pred_label)
...
def train(args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        #all
        if (epoch % 2) == 0 or epoch == args.n_epoch:
            time66 = time()
            auc, precision, recall, f1, accuracy = evaluate_all(model, dataloader=data, device=device)
            logging.info(
                'CF Evaluation: Epoch {:04d} | Total Time {:.1f}s | Precision {:.4f}, Recall {:.4f}, f1 {:.4f},auc {:.4f},acc {:.4f}'.format(
                    epoch, time() - time66, precision,recall,f1,auc,accuracy))
...
def get_id(args):
    ...
    while True:
        ...
        try:
            ...
            scores = model(torch.LongTensor([user_id]).to(device), item_ids, mode='predict')

            scores = scores.detach().cpu().numpy()
            itemids = item_ids.detach().cpu().numpy()
            dic = dict(zip(itemids, scores[0]))
