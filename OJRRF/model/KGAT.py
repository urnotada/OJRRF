...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
        ...
        # self.user_emb=utils_user.get_emb_dict(self.device)
        # self.W=nn.Linear(args., args.hidden_size)
        self.w = torch.nn.Sequential(
            torch.nn.Linear(240,176),
            torch.nn.ReLU(),
            torch.nn.Linear(176, 176),
        )
    ...
    def calc_cf_embeddings(self):
        ...
        for idx, layer in enumerate(self.aggregator_layers):
            ...
            norm_embed = F.normalize(ego_embed, p=1, dim=1)
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        if flag:
            ...
            user_word2vec = utils_user.get_emb(user_ids.tolist(), self.device)

            user_embed = torch.cat((user_embed, user_word2vec), dim=1)
            # 走一个全连接
            # user_embed=nn.Linear(user_embed.shape[1], all_embed.shape[1])(user_embed)

            user_embed=self.w(user_embed)
    ...
    def update_attention(self, h_list, t_list, r_list, relations):
        ...
        df=pd.DataFrame(columns=['h','t','w'])
        df['h']=rows.detach().cpu().numpy()
        df['t']=cols.detach().cpu().numpy()
        df['w']=F.softmax(values).detach().cpu().numpy()
