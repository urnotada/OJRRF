...
class DataLoaderBase(object):
    ...
    # Load text vector dictionary
    def get_emb_dict(self):
        # read file
        with open("./datasets/job/concatenated.pkl", "rb") as tf:
            emb_dict = pickle.load(tf)
        return emb_dict

    # Build text embs
    def get_emb(self,user_ids):
        emb_dict = self.get_emb_dict()
        embs = []
        for i in user_ids.numpy().tolist():
